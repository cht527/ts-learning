...
class MultiRequest{
    public reqList:string[]=[];
    public limit:number;
    private count:number=0;
    private lock:Array<(value?: unknown) => void>=[];
    constructor(urlList:string[],limit:number){
    ...
    async call(){
        ...
        if(this.reqList.length>0){
            this.count++;
            const res=await axios.get(this.reqList.shift() as string);
