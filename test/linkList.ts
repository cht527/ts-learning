...
class LinkedList<T>{
    ...
    insert(index:number, ele:T){
        ...
        if(index === 0 || this.isEmpty){
            ...
            node.next = (targetNode as INode<T>).next;
            (targetNode as INode<T>).next = node
        }
    ...
    remove(index:number){
        if(index >= 0 && index<this.length){
            ...
            if(index===0 && headNode!==null){
                ...
                (prevNode as INode<T>).next = currentNode.next;
...
const linkedNode = new LinkedList<number>();
